...
def set_task_version(task: dict):
    ...
    try:
        ...
        # tasks_df was already fetched successfully, so this isn't Elastic connectivity - it's a
        # bug (e.g. a non-numeric task_version). Still don't abort the whole generate_tasks() batch
        # over one task's version, but log it loudly so it doesn't read as routine flakiness.
...
def _get_matching_tasks(task: dict) -> pd.DataFrame:
    ...
    # Filter out non-integer version values. Only touch the column once we know there are hits -
    # on zero hits, get_docs_by_query returns a columnless DataFrame and indexing it would raise.
