...
def load_files_from_local_storage(input_file_set):
    ...
    for file_name in pypowsybl_file_list:
        ...
        contents = get_metadata_from_file_name(base_path)
...
def export_statistics(merged_model, input_models, ac_schedules, dc_schedules, schedule_time_horizon, schedule_start, sv_data, ssh_data):
    ...
    if generate_statistics:
        ...
        report_name = Path(__file__).parent / f"statistics-{time_moment_now}.xlsx"
        # report_name = report_name.removesuffix('/') + '/' + f"stat_{merged_model.name}_{schedule_time_horizon}.xlsx"
        models_as_triplets = load_opdm_objects_to_triplets(input_models)

        with pandas.ExcelWriter(report_name, engine="xlsxwriter") as excel_writer:
            ...
            worksheet.write_string(row_counter, col_counter,
                                   f'AC schedules: {schedule_start}_{schedule_time_horizon}')
            row_counter = row_counter + 1
            ac_schedules_df.to_excel(excel_writer, sheet_name=sheet_name, startrow=row_counter,
                                     startcol=col_counter)
            row_counter = row_counter + 3 + len(ac_schedules_df)

            worksheet.write_string(row_counter, col_counter,
                                   f'DC schedules: {schedule_start}_{schedule_time_horizon}')
            row_counter = row_counter + 1
            dc_schedules_df.to_excel(excel_writer, sheet_name=sheet_name, startrow=row_counter,
                                     startcol=col_counter)
