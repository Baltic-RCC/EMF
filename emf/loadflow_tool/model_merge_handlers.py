...
class HandlerGetModels:
    ...
    def handle(self, *args, **kwargs):
        ...
        if input_data := cgm_input.task_data:
            ...
            available_tso = [model["pmd:TSO"] for model in available_models]
            missing_tso = [tso for tso in included_tsos if tso not in available_tso]

            if missing_tso:
                replacement_models = run_replacement(tso_list=missing_tso,
                                                     time_horizon=cgm_input.time_horizon,
                                                     scenario_date=cgm_input.scenario_date)
                if not replacement_models:
                    logger.warning("Replacement models not found")
                else:
                    available_models.append(replacement_models)
